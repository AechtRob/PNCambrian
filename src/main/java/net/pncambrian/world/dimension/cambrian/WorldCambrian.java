...
public class WorldCambrian extends ElementsPNCambrianMod.ModElement {
	public static int DIMID = LepidodendronConfig.dimCambrian;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("lepidodendron:cambrian_portal")
	public static final WorldCambrian.BlockCustomPortal portal = new WorldCambrian.BlockCustomPortal();
	public static DimensionType dtype;
	public WorldCambrian(ElementsPNCambrianMod instance) {
		super(instance, 2);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new WorldCambrian.BlockCustomPortal());
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		dtype = DimensionType.register("cambrian", "_cambrian", DIMID, WorldCambrian.WorldProviderMod.class, false);
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		@Override
		public void init() {
			this.biomeProvider = new BiomeProviderCambrian(this.world.getSeed(), this.world.getWorldInfo());
		...
		public Biome getBiomeForCoordsBody(final BlockPos pos, World worldIn)
		{
			...
			{
				return worldIn.provider.getBiomeProvider().getBiome(pos, BiomeCambrianSeaShore.biome);
			}
		}
		...
		public Biome getBiome(Chunk chunk, BlockPos pos, BiomeProvider provider)
		{
			...
			if (k == 255)
			{
				...
				Biome biome = world.isRemote ? BiomeCambrianSeaShore.biome : provider.getBiome(pos, BiomeCambrianSeaShore.biome);
				k = Biome.getIdForBiome(biome);
				chunk.getBiomeArray()[j << 4 | i] = (byte)(k & 255);
			}

			Biome biome1 = Biome.getBiome(k);
			return biome1 == null ? BiomeCambrianSeaShore.biome : biome1;
		}
		...
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderCambrian(this.world, this.world.getSeed() - DIMID);
		}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setTranslationKey("pf_cambrian_portal");
			setRegistryName("lepidodendron:cambrian_portal");
			setLightLevel(0F);
		}
		...
		public boolean portalSpawn(World worldIn, BlockPos pos) {
			WorldCambrian.BlockCustomPortal.Size portalsize = new WorldCambrian.BlockCustomPortal.Size(worldIn, pos, EnumFacing.Axis.X);
			if (portalsize.isValid() && portalsize.portalBlockCount == 0) {
				portalsize.placePortalBlocks();
				return true;
			} else {
				portalsize = new WorldCambrian.BlockCustomPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
				if (portalsize.isValid() && portalsize.portalBlockCount == 0) {
					portalsize.placePortalBlocks();
					return true;
				}
		...
		public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
			...
			WorldCambrian.BlockCustomPortal.Size blockportal$size = new WorldCambrian.BlockCustomPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
			LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);
			if (!blockportal$size.isValid()) {
				enumfacing$axis = EnumFacing.Axis.X;
				blockportal$size = new WorldCambrian.BlockCustomPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
			}
			if (!blockportal$size.isValid()) {
		...
		public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				Minecraft.getMinecraft().effectRenderer.addEffect(ParticlePNPortal.PortalParticleFactory.createParticle(world, d0, d1, d2, d3, d4, d5, 0.2852F, 0.5312F, 0.2188F));
			}
		}
			...
			if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
				...
				if (thePlayer.timeUntilPortal > 0) {
					...
					if (thePlayer.dimension == DIMID) {
						...
						ModTriggers.ENTER_CAMBRIAN.trigger((EntityPlayerMP)thePlayer);
					}
		...
		public static WorldCambrian.TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, int dimid) {
			...
			return new WorldCambrian.TeleporterDimensionMod(entity.getServer().getWorld(dimid), new Vec3d(d1, d2, 0), bph.getForwards());
		}
