...
public class BiomeCambrianSeaReefs extends ElementsPNCambrianMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:cambrian_sea_reefs")
	public static final BiomeGenCustom biome = null;
	public BiomeCambrianSeaReefs(ElementsPNCambrianMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomeCambrian {
		public BiomeGenCustom() {
			super(new BiomeProperties("Cambrian Ocean Reefs").setRainfall(0.9F).setBaseHeight(-1.0F).setHeightVariation(0.16F).setTemperature(0.9F));
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int ii = 0; ii < 20; ++ii)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getSeaLevel() + 25);
					STATIC_GENERATOR.generate(BlockGogia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 1, 100, 0, 255);
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int ii = 0; ii < 20; ++ii)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getSeaLevel() + 25);
					STATIC_SIDEWAYS_GENERATOR.generate(BlockGogia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 1, 100, 0, 255);
				}
