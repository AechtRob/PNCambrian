...
import net.pncambrian.world.dimension.cambrian.WorldCambrian;
    ...
    public void goToNether(EntityTravelToDimensionEvent event) {
        if (LepidodendronConfig.oneWayPortalsNether && LepidodendronConfig.oneWayPortals) {
            if (event.getDimension() == -1) {
                ...
                if (event.getEntity().getEntityWorld().provider.getDimensionType().getId() == LepidodendronConfig.dimCambrian) {
                    if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
                        ...
                        if (thePlayer.dimension != event.getDimension()) {
                            ...
                            WorldCambrian.BlockCustomPortal.transferPlayerToDimensionPN(thePlayer.server.getPlayerList(), thePlayer, event.getDimension(), WorldCambrian.BlockCustomPortal.getTeleporterForDimension(thePlayer, entityIn.getPosition(), event.getDimension()));
                        }
                    }
                    event.setCanceled(true);
                }
